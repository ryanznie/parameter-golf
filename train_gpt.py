...
class Hyperparameters:
    ...
    mlp_leak = float(os.environ.get("MLP_LEAK", 0.5))
...
class Rotary(nn.Module):
    # Caches RoPE tables per sequence length and supports applying RoPE
    # to only a prefix of each head for a cheap partial-positional variant.
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if rope_dims > self.head_dim or rope_dims % 2 != 0:
            raise ValueError(f"rope_dims must be an even value in [2, head_dim], got {rope_dims}")
...
class MLP(nn.Module):
    ...
    def __init__(self, dim: int, mlp_mult: int, leak: float):
        super().__init__()
        hidden = mlp_mult * dim
        self.leak = leak
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = F.leaky_relu(self.fc(x), negative_slope=self.leak)
        return self.proj(x.square())


class SmearGate(nn.Module):
    # Blend a small amount of the previous token representation into the current
    # token before the transformer, giving cheap local n-gram context.
...
class BigramHashEmbedding(nn.Module):
    # Hash consecutive token pairs into a compact learned table, which helps
    # recover some local lexical statistics lost by the tiny tokenizer.
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(dim, mlp_mult, mlp_leak)
...
def main() -> None:
    ...
    log0(
        ...
        f"xsa_last_n:{args.xsa_last_n} ln_scale:{args.ln_scale} mlp_leak:{args.mlp_leak}"
    )
    log0(
        f"tie_embeddings:{args.tie_embeddings} embed_lr:{token_lr} "
        f"head_lr:{args.head_lr if base_model.lm_head is not None else 0.0} "
        f"matrix_lr:{args.matrix_lr} scalar_lr:{args.scalar_lr} "
        f"adam_wd:{args.adam_wd} muon_wd:{args.muon_wd} ema_enabled:{args.ema_enabled}"
    )
    log0(
    ...
    ema_state = {name: tensor.detach().float().clone() for name, tensor in base_model.state_dict().items()} if args.ema_enabled else None
